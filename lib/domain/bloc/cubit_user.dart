...
class UserCubit extends Cubit<UserState> {
  ...
  void startingState() async {
    try {
      ...
      final data = await databaseClient.fetchState(userModel.objectId);
  ...
  void sendRequest() async {
    try {
      await databaseClient.sendRequest(userModel.objectId);
      emit(UserPending());
    } on String catch (error) {
      emit(UserError(error));
    }
  ...
  void acceptRequest() async {
    try {
      await databaseClient.acceptRequest(userModel.friendRequestId);
      emit(UserFriend());
    } on String catch (error) {
      emit(UserError(error));
    }
  ...
  void rejectRequest() async {
    try {
      await databaseClient.rejectRequest(userModel.friendRequestId);
      emit(UserRandom());
    } on String catch (error) {
      emit(UserError(error));
    }
  ...
  void unfriend() async {
    try {
      await databaseClient.unfriendUser(userModel.friendRequestId);
      emit(UserRandom());
    } on String catch (error) {
      emit(UserError(error));
    }
